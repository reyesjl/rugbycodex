  ...
  <section class="base-container container flex min-h-screen flex-col gap-16 pt-32 pb-32">
    <header
      ...
      <p class="mt-4 max-w-xl text-neutral-600 dark:text-neutral-400">
        Manage and oversee all user profiles within the Rugbycodex platform.</p>
    </header>
    ...
    <section
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          ...
          <button type="button" @click="handleRefresh" :disabled="profilesLoading"
      ...
      <div v-else class="scrolling-list mt-4 max-h-[60vh] space-y-4 overflow-y-auto pr-2">
        <TransitionGroup name="profile-list" tag="div" class="space-y-4">
          <div v-for="profile in filteredProfiles" :key="profile.id"
            ...
            <div @click="toggleExpand(profile.id)"
              ...
              <div class="flex-1">
                <RouterLink :to="`/admin/profiles/${profile.id}`" @click.stop
                  class="text-neutral-900 dark:text-neutral-100 hover:underline font-medium">
                  {{ profile.name }}
                </RouterLink>
              </div>
              <div class="flex items-center gap-2">
            ...
            <div class="grid transition-all duration-300 ease-in-out"
              ...
              <div class="overflow-hidden">
                <div class="space-y-2 border-t border-neutral-200/60 pt-3 pb-3 px-3 dark:border-neutral-800/70">
                  ...
                  <p class="text-sm text-neutral-600 dark:text-neutral-400">Created: {{ profile.creation_time.toLocaleString() }}</p>
                </div>
              </div>
            </div>
          </div>
