...
export const assignmentsService = {
  ...
  async getManagerAssignments(
    ...
    // Use optimized RPC function to reduce 6+ queries to 1
    const { data, error } = await supabase.rpc('rpc_get_manager_assignments', {
      p_org_id: orgId,
      p_limit: limit,
    });
    ...
    // Group rows by assignment_id (RPC returns one row per assignment-target pair)
    const assignmentMap = new Map<string, ManagerAssignment>();
    
    for (const row of data) {
      const assignmentId = row.assignment_id;
      
      if (!assignmentMap.has(assignmentId)) {
        // First time seeing this assignment, create it
      ...
      // Add target to assignment
      const assignment = assignmentMap.get(assignmentId)!;
      assignment.targets.push({
        type: row.target_type as AssignmentTargetType,
