...
const myMemberships = useMyOrganizationsStore();
const prioritizedMemberships = computed(() => {
  return [...myMemberships.items].sort((a, b) => {
    const aOrder = ROLE_ORDER[a.membership.role] ?? Number.MAX_SAFE_INTEGER;
    const bOrder = ROLE_ORDER[b.membership.role] ?? Number.MAX_SAFE_INTEGER;
...
<template>
  <section class="space-y-6">
    <article
      ...
      <div v-if="myMemberships.items.length === 0" class="mt-4 text-sm text-neutral-600 dark:text-neutral-400">
        You are not a member of any organizations.
      </div>
      <div v-else class="max-h-[80vh] overflow-y-auto">
        <div v-for="m in prioritizedMemberships" :key="m.organization.id"
          ...
          <div class="flex items-center justify-between">
            <AnimatedLink :to="`/organizations/${m.organization.slug}`" :text="m.organization.name" class="text-base font-medium"
              @click.stop />
            ...
            <div class="flex overflow-auto items-center gap-4 text-sm text-neutral-600 dark:text-neutral-400">
              ...
              <span>
                {{ m.membership.joined_at?.toLocaleDateString() }}
              </span>
            </div>
          </div>
        </div>
